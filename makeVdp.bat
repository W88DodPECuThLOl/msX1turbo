...
set BASE_DIR=%~dp0
set TARGET=%BASE_DIR%vdp.mod
set PACKAGE=

set OBJ_DIR=%BASE_DIR%obj
set SRC_DIR=%BASE_DIR%src\moduleVDP
...
set C=sdcc
set C_FLAGS=-mz80 --debug --asm=asxxxx --std-c23 --disable-warning 110 -I%CAT_Z80_LIB_INCLUDE% -DVRAM_8000=1
...
set ASM=sdasz80
set ASM_FLAGS=-l -s -I"%BASE_DIR%src\common" -I"%BASE_DIR%src\common\turbo"
...
set LINK=sdcc
REM set LINK_FLAGS=-mz80 --out-fmt-ihx --no-std-crt0 --code-loc 0xC000 --data-loc 0
set LINK_FLAGS=-mz80 --out-fmt-ihx --no-std-crt0 --code-loc 0xC000 --data-loc 0
...
:link
    @echo Link "%TARGET%"
    set objFiles=
    if exist "%OBJ_DIR%\x1_custom_crt0.rel" (
    ...
    makebin -p -s 65536 -o 49152 "%OBJ_DIR%\temp.ihx" %TARGET%
exit /b %errorlevel%
